...
use collections::{BTreeIndex, EqIndex, HashMapIndex, Index, NoIndex, RadixTreeIndex, RudyMapIndex,
                  SeqIndex, SortedVecIndex, Table};
use pstr::IStr;
...
type String = SmartString<Compact>;
//type String = IStr;
...
fn bench_control<VALUE: Clone + Ord + Hash>(data: &[VALUE],
                                            sizes: &[usize],
                                            log: &mut Vec<BenchResult>)
{
    log.append(&mut bench_seq_index::<VALUE, NoIndex>(data, &sizes[0..2]));
}

fn bench_eq_indexes<VALUE: Clone + Ord + Hash>(data: &[VALUE],
                                               sizes: &[usize],
                                               log: &mut Vec<BenchResult>)
{
...
fn bench_fk_indexes<VALUE: rudy::Key + Hash>(data: &[VALUE],
                                             sizes: &[usize],
                                             log: &mut Vec<BenchResult>)
{
...
fn bench_seq_indexes<VALUE: Clone + Ord + Hash + Send>(data: &[VALUE],
                                                       sizes: &[usize],
                                                       log: &mut Vec<BenchResult>)
{
...
fn bench_seq_index<VALUE: Clone + Ord + Hash, INDEX: SeqIndex<VALUE>>(data: &[VALUE],
                                                                      sizes: &[usize])
                                                                      -> Vec<BenchResult>
{
...
fn bench_eq_index<VALUE: Clone + Ord + Hash, INDEX: EqIndex<VALUE>>(data: &[VALUE],
                                                                    sizes: &[usize])
                                                                    -> Vec<BenchResult>
{
    let mut performance = vec![];
...
fn bench_eq<VALUE: Clone + Ord + Hash, INDEX: EqIndex<VALUE>>(data: &[VALUE],
...
fn bench_seq<VALUE: Ord + Clone + Hash, INDEX: SeqIndex<VALUE>>(data: &[VALUE],
                                                                table: &Table<INDEX, VALUE>)
                                                                -> BenchResult
{
    let (_, mem, time) = benchit(|| {
    ...
    let subject = table.name();
    return BenchResult { subject,
                         test: "sequential 100x100",
                         size: data.len(),
                         mem,
                         time };
}

fn bench_ins<VALUE: Clone + Ord + Hash, INDEX: Index<VALUE>>(
    data: &[VALUE])
    -> (Table<INDEX, VALUE>, BenchResult)
{
