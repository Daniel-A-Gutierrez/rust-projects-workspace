...
mod test
{
    ...
    mod tests
    {
        ...
        fn s_tree_negative()
        {
            ...
            {
                let d = &odds[i];
                println!("{}\t{}\t{:?}\t{:?}\t{:?}", i, d ,evens_tree.find_first(d), evens_tree.find_last(d), evens_tree.find_range(d));
            }
            println!();
            println!("{:?}", (0..odds.len()).into_iter().collect::<Vec<usize>>());
            println!("{:?}",odds);
            ...
            {
                let d = &evens[i];
                println!("{}\t{}\t{:?}\t{:?}\t{:?}", i, d ,odds_tree.find_first(d), odds_tree.find_last(d), odds_tree.find_range(d));
            }
        }
...
//where u32 : Clone + Eq + Ord + Sized + Sync + Send
{
    fn from_unsorted(src : &[u32]) -> Result<Self>
    {
        ...
        else { tree.sort_by(|a,b| b.cmp(a)); }
        let mut tier_len = src.len()/DEGREE;
        let mut step_by = DEGREE;
        for _ in 1..height //could be improved by just going over previous tier.
        {
            let mut s = step_by-1;
            for _ in (0..tier_len)
            {
