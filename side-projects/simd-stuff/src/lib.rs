...
#[inline]
fn compare_arrays4<const VLEN : usize>(a : &[i32;1024], b : &[i32;1024]) 
    -> (i32, i32, i32)
...
impl IStringPool
{
    pub fn new() -> Self 
    {
        return Self { previews: vec![], strings: vec![], len: 0 };
    }
    ...
    unsafe fn make_preview(s : &str) -> u64
    {
        ...
        for (i,byte) in s.as_bytes().iter().take(8).enumerate() //8th is null terminator
        {
            preview[7-i]=*byte;
        }
...
where LaneCount<VLEN> : SupportedLaneCount
{
    ...
    for (chunk_idx,schunk) in tests.previews.array_chunks::<VLEN>().enumerate()
    {
        ...
        while (eqmask.any() && needle.len() > 8) //if needle is 8 characters or less and matches, hay cant be less than it. 
        {
            let fs = eqmask.first_set().unwrap(); //just checked
            eqmask.set(fs, false);
            if let Ordering::Less = compare_strings::<VLEN>(&tests.strings[chunk_idx * VLEN +fs], needle)
            {
                ltmask.set(fs,true);
            }
        }
