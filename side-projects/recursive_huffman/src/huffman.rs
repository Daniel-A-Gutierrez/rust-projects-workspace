...
use bit_vec;
use bitstream_io::{BigEndian, BitReader, BitWriter, BitWrite, BitRead};
...
impl HuffmanTree {
    ...
    pub fn from_table(data: &[u64;256]) -> Self {
    ...
    pub fn from_data(data: &[u8]) -> Self {
        ...
        HuffmanTree::from_table(&probability)
    }
    ...
    /// 
    /// also poorly designed because its nlogn when it only has to be n
    /// 
    /// fixed it
    pub fn to_table(&self) -> [u64; 256] {
    ...
    /// lowest probability
    pub fn get_byte_prob(&self, byte: u8) -> Option<u64> {
        match self {
    ...
    // fn normalize(data: &[usize], max_elem: usize) -> [usize; 256] {
...
impl<W> Write for HuffmanWriter<W> where W: Write {
    ///maps u8's to their encoding. pads end of stream with 0s to a whole byte.
...
///the previous reader didnt give results, so now this one handles errors badly.
