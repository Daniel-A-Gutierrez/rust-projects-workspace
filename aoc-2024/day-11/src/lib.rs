...
fn recursive_helper(memos: &mut HashMap<(u64, u32), u64>, stone: u64, iter: u32, limit: u32)
                    -> u64
{
    if (iter >= limit)
    {
    ...
    if let Some(value) = memos.get(&(stone, iter))
    {
        return *value;
    }
...
fn split_number(u: u64) -> (u64, u64)
{
    ...
    assert!(digits % 2 == 0,
            "Cannot split a number with an odd number of digits.");
...
fn load_input() -> Result<Vec<u64>>
{
    let mut file = fs::OpenOptions::new().read(true)
                                         .open("aoc-2024/day-11/input.txt")?;
    let mut text = String::new();
    file.read_to_string(&mut text)?;
    let stone_inscriptions = text.split(" ");
    let stones: Vec<u64> = stone_inscriptions.map(|i| {
                                                 i.parse()
